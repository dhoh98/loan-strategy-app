...
def plot_radar(score_equal, score_principal):
    ...
    ax.plot(angles, values_equal, label="원리금균등", marker='o')
    ax.plot(angles, values_principal, label="원금균등", marker='o')
...
if analyze_btn:
    ...
    with col1: kpi_card("원리금 총 이자", f"{int(total_equal):,} 원")
    with col2: kpi_card("원금균등 총 이자", f"{int(total_principal):,} 원")
    with col3: kpi_card("AI 추천 전략", f"{recommended}")
